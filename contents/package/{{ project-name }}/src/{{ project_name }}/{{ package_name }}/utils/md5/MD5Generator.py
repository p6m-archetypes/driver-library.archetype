...
class MD5Source:
    ...
    def exclude_keys(self, json_data):
        ...
        if isinstance(json_data, dict):
            output = {key: self.exclude_keys(value) for key, value in json_data.items() if
                      key not in self.keys_to_exclude}
            return output
        elif isinstance(json_data, list):
            output = [self.exclude_keys(element) for element in json_data]
            return output
        else:
            return json_data
    ...
    def sort_keys_and_values(self, json_data):
        ...
        if isinstance(json_data, dict):
            return {key: self.sort_keys_and_values(value) for key, value in sorted(json_data.items())}
        elif isinstance(json_data, list):
    ...
    def concatenate_keys_and_values(self, json_data):
        ...
        if isinstance(json_data, dict):
            return self.delimiter.join(
                f'{key}{self.delimiter}{self.concatenate_keys_and_values(value)}' for key, value in json_data.items())
        elif isinstance(json_data, list):
            return self.delimiter.join(self.concatenate_keys_and_values(element) for element in json_data)
        else:
    ...
    def process_json(self):
        ...
        print(f'digest of = {lowercase_str}')
    ...
    def generate_md5_hash(self):
        ...
        (_, _, _, _, hash_id) = self.process_json()
        return hash_id
